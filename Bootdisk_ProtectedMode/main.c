...
void print_string(const char* str) {
	// pointer to VGA-text-memory (each char takes 2 Byte: 1x ASCII, 1x Attribute)
    ...
    while (*str) {
        *video_memory = *str;          // Zeichen schreiben
        *(video_memory + 1) = 0x0F;    // Attribut-Byte: Weißer Text auf schwarzem Grund
        
        str++;
        video_memory += 2;             // Zum nächsten Zeichen springen
    }
}
...
__attribute__((noreturn)) void main() {
    print_string("Hello World from C-Code in 32-bit protected mode!");
